...
import { CreatePropertyDto, PropertyType, PropertyStatus, PropertyTranslation } from '@/types/property';
...
export function CreatePropertyModal({
  ...
  const handleSubmit = async () => {
    ...
    if (formData.price <= 0) {
      toast({
        title: tCommon('error') || 'Error',
        description: t('errors.priceRequired') || 'Price must be greater than 0',
        variant: 'error',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: t('createSuccess') || 'Property created successfully',
      });
      onSuccess();
    } catch (error) {
      ...
      toast({
        ...
        description: error instanceof PropertyApiError 
          ? error.message 
          : t('errors.createFailed') || 'Failed to create property',
        variant: 'error',
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('createProperty') || 'Create Property'}</DialogTitle>
          <DialogDescription>
            {t('createPropertyDescription') || 'Add a new property to your portfolio'}
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="details" className="space-y-4 mt-4">
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  type="number"
                  value={formData.details?.surfaceArea || ''}
            ...
            <div className="text-sm text-gray-500">
              {t('form.detailsNote') || 'Additional details can be added after creation'}
            </div>
          </TabsContent>
