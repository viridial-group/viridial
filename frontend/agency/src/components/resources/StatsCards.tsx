...
import { Folder, CheckCircle2, Shield, TrendingUp } from 'lucide-react';
...
export const StatsCards = memo(function StatsCards({ resources }: StatsCardsProps) {
  const t = useTranslations('admin');
  
  const stats = useMemo(() => {
    const total = resources.length;
    const active = resources.filter((r) => r.isActive).length;
    const totalPermissions = resources.reduce((sum, r) => sum + (r.permissions?.length || 0), 0);
  ...
  const cards = [
    {
      label: t('resources.totalResources') || 'Total Resources',
      value: stats.total,
      icon: Folder,
      color: 'text-blue-600',
      bgColor: 'bg-blue-50',
    ...
    {
      label: t('resources.activeResources') || 'Active Resources',
      value: stats.active,
      icon: CheckCircle2,
    ...
    {
      label: t('resources.totalPermissions') || 'Total Permissions',
    ...
    {
      label: t('resources.avgPermissions') || 'Avg Permissions',
