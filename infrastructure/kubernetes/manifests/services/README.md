# Manifests Kubernetes - Services de Base (Production uniquement)

Ce répertoire contient les manifests Kubernetes pour déployer les services de base de Viridial (US-INFRA-02) en **production uniquement**.
...
│   └── minio-ingress.yaml            # Ingress pour console et API
...
**Note:** Le script déploie uniquement en production (`viridial-production`).
...
kubectl wait --for=condition=complete job/minio-init-buckets -n viridial-production --timeout=120s
```
...
```bash
# Vérifier les pods
kubectl get pods -n viridial-production

# Vérifier les services
kubectl get svc -n viridial-production

# Vérifier les PVC
kubectl get pvc -n viridial-production

# Vérifier les logs
kubectl logs -l app=postgres -n viridial-production
kubectl logs -l app=redis -n viridial-production
kubectl logs -l app=meilisearch -n viridial-production
kubectl logs -l app=minio -n viridial-production
```
...
- **PostgreSQL:** `postgres.viridial-production.svc.cluster.local:5432`
- **Redis:** `redis.viridial-production.svc.cluster.local:6379`
- **Meilisearch:** `http://meilisearch.viridial-production.svc.cluster.local:7700`
- **MinIO API:** `http://minio.viridial-production.svc.cluster.local:9000`
- **MinIO Console:** `http://minio.viridial-production.svc.cluster.local:9001`
...
kubectl describe pod <pod-name> -n viridial-production

# Vérifier les PVC
kubectl get pvc -n viridial-production
kubectl describe pvc <pvc-name> -n viridial-production
```
...
kubectl get secrets -n viridial-production
kubectl describe secret <secret-name> -n viridial-production
```

## Scripts Utiles

- `deploy-base-services.sh` - Déploiement automatisé
- `test-services-connectivity.sh` - Tests de connectivité
- `diagnose-services.sh` - Diagnostic complet
- `fix-resource-limits.sh` - Correction des limites de ressources
